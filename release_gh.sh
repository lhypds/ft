#!/usr/bin/env bash
# Publish an ft release zip to GitHub (lhypds/ft) using the `gh` CLI.
# Usage:
#   ./release_gh.sh                          # derive tag + zip path from VERSION
#   ./release_gh.sh v0.0.2                   # explicit tag, derive zip path
#   ./release_gh.sh v0.0.2 path/to/ft.zip    # explicit tag and zip path
set -euo pipefail
...
if [ $# -ge 2 ]; then
    ...
    VERSION="$1"
    BARE="${VERSION#v}"
    ZIP_PATH="$RELEASE_DIR/ft_v${BARE}.zip"
else
