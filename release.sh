...
# GitHub (lhypds/ft). The archive contains everything `install.sh` needs to set
# up a fresh checkout: the `ft/` package, top-level scripts, requirements, etc.
set -euo pipefail
...
# Package source
cp -R "$ROOT_DIR/ft" "$STAGING_DIR/ft"

# Top-level scripts and project files
cp "$ROOT_DIR/ft.py"            "$STAGING_DIR/"
