"""Where ``ft`` keeps its settings, and how it asks for a missing API key.

Settings live in ``~/.config/ft/.env`` (``$XDG_CONFIG_HOME/ft/.env`` when that
variable is set), so an installed ``ft`` finds its keys no matter which
...
``ft`` was installed outside a checkout.
...
2. a ``.env`` beside a development checkout, so hacking on the repo needs no
   edits to ``~/.config``
3. ``~/.config/ft/.env``, written by ``setup.sh`` and by the prompt below
"""
...
def config_dir() -> Path:
    """The directory holding ``ft``'s settings."""
...
def load_config() -> None:
    ...
    # order gives: real environment > checkout .env > ~/.config/ft/.env.
...
def _ask_for_key(name: str, purpose: str, note: str) -> str:
    ...
    ``ft`` is part of a pipeline — and so a piped stdin is never eaten.
    """
    try:
        tty = open("/dev/tty", "r")
    except OSError:
