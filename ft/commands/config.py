"""Open ft's settings file in an editor.
...
Usage:
    ft config           # edit the settings file
    ft config --path    # print its path (for scripts: "$(ft config --path)")
    ft config --show    # list which keys are set, without opening an editor
"""
...
TEMPLATE = """\
# Used by `ft search` — the default search backend.
BRAVE_API_KEY=
# Used by `ft summarize`, and by `ft search` when BRAVE_API_KEY is empty.
OPENAI_API_KEY=
"""

# Why each key matters, shown by --show and after an edit.
KEY_PURPOSE: dict[str, str] = {
    "OPENAI_API_KEY": "required for `ft summarize`, and for `ft search` without a Brave key",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="ft config",
        description="Open ft's settings file (API keys) in your editor.",
    )
    parser.add_argument(
