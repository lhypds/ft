"""Update ft to the latest GitHub release.

`get.sh` on main is the same installer the README tells users to curl, and it
...
Usage:
    ft update              # update when a newer release exists
    ft update -f|--force   # re-run the installer even when up to date
"""
...
LAUNCHER = Path.home() / ".local" / "bin" / "ft"
LAUNCHER_MARKER = "# ft-launcher:REPO="
...
DEFAULT_INSTALL_DIR = _DATA_HOME / "ft"
LEGACY_INSTALL_DIR = Path.home() / ".ft"  # where earlier releases unpacked to

USER_AGENT = "ft-updater"
...
def install_dir() -> Path:
    ...
    # An install still sitting at the pre-XDG ~/.ft is named as the new default
...
def run_installer(version: str, target: Path) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ft-update-") as tmp:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="ft update",
        description="Update ft to the latest GitHub release.",
    )
    parser.add_argument(
        "-f",
        "--force",
